...
// Define the delayed_reset function
gboolean delayed_reset(gpointer data) {
...
void button_clicked(GtkWidget *button, gpointer data) {
    ...
    if (board[index] == ' ') {
        board[index] = player;
        char label[2] = {player, '\0'};
        gtk_button_set_label(GTK_BUTTON(button), label);
        if (player == 'X') {
            gtk_widget_set_name(button, "button-x");
        } else {
            gtk_widget_set_name(button, "button-o");
        }

        if (checkWin(player)) {
            char message[50];
            snprintf(message, sizeof(message), "Player %c wins!", player);
            if (player == 'X') {
                score_x++;
            } else {
                score_o++;
            ...
            g_timeout_add_seconds(2, delayed_reset, NULL); // Schedule a delayed reset after 2 seconds for a win
            ...
            score_ties++;
            g_timeout_add_seconds(2, delayed_reset, NULL); // Schedule a delayed reset after 2 seconds for a draw
            gtk_label_set_text(GTK_LABEL(status_label), message);
            update_scoreboard();
            gtk_widget_set_sensitive(end_button, FALSE);
            for (int i = 0; i < 9; i++) {
