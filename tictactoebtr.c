...
// Minimax algorithm
int minimax(char player, bool isMaximizing) {
    ...
    for (int i = 0; i < 9; i++) {
        if (board[i] == '-') {
            board[i] = player;
            int score = minimax(player == 'O' ? 'X' : 'O', !isMaximizing);
            board[i] = '-';
...
void takeTurn(char player) {
    ...
    // printBoard(); // Removed to avoid double printing
}
